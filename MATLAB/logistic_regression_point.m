%% point_logistic_regression_9_maps_select_files.m
% Punktbasierte logistische Regression mit 9 schrittweisen Modellen
...
%   3) GeoTIFF der modellierten Wahrscheinlichkeit
%
% Keine geglaettete Karte.
% Keine Histogramme.
% Keine Koeffizientenplots.
%
% Wichtig:
...
fprintf('\nHangpixel-Tabelle: %d Zeilen\n', height(H_raw));
fprintf('Random-Tabelle:    %d Zeilen\n', height(R_raw));

disp('Spalten Hangpixel:');
...
H = makeModelTable(H_raw, 1, "mapped");
R = makeModelTable(R_raw, 0, "random");
...
xlabel('Modell');
ylabel('AIC');
title('AIC comparison - point-based logistic regression', ...
    'Interpreter','none');

grid on;

exportgraphics(fig, fullfile(outDir, ...
    'AIC_comparison_point_based_logistic_regression.png'), ...
    'Resolution', 300);
...
title('Coefficients of best point-based logistic model', ...
    'Interpreter','none');

grid on;

exportgraphics(fig, fullfile(outDir, ...
    'coefficients_best_point_based_logistic_model.png'), ...
    'Resolution', 300);
...
title(sprintf('ROC curve - best point-based logistic model (AUC = %.4f)', AUC), ...
    'Interpreter','none');
grid on;
axis square;

exportgraphics(fig, fullfile(outDir, ...
    'ROC_best_model_AUC.png'), ...
    'Resolution', 300);
close(fig);
...
fprintf('Maximum P: %.4f\n', max(P(:), [], 'omitnan'));
    ...
    title({sprintf('Model_%d: punktbasierte logistische Regression', k), ...
           'fein abgestufte modellierte Anfälligkeit'}, ...
           'Interpreter','none');

    exportgraphics(fig, fullfile(outDir, ...
        sprintf('model_%02d_probability_fine_blue_yellow_red.png', k)), ...
        'Resolution', 300);
    close(fig);
    ...
    exportgraphics(fig, fullfile(outDir, ...
        sprintf('model_%02d_probability_classes_5_blue_yellow_red.png', k)), ...
        'Resolution', 300);
    close(fig);
